...
void readSpectrometer()
{
  if (EnableScan==true)
  {  
    int delayTime = 1;                // delay time, the normal execution speed adds an additional 1us
  ...
  // Read from SPEC_VIDEO, data starts on 89th clk after Start goes low
        ...
        {
        if (ac>0)                                    // do one dummy scan, do not save data
        {
          if (sensor<1)                              // check which sensor to scan
          {        
            dataUp[i] += adc1_get_raw(ADC1_CHANNEL_0);    // requires adc.h, 33us
///            dataUp[i] += analogRead(SPEC_VIDEO_UP);    // read ADC Up data into array and sum next sample
          }
          else
          {
            dataDwn[i] += adc1_get_raw(ADC1_CHANNEL_1);   // requires adc.h, 33us
///            dataDwn[i] += analogRead(SPEC_VIDEO_DWN);  // read ADC Down data into array and sum next sample 
          }                                            // total time is (one clock + ADC conversion time, 100us)
        }                                              // * channel bins
  ...
  // calculate average scan values    
    ...
    EnableScan = false;                         // disable scan
    //Serial.println("G");                        // send go command that data is available
  }
}
//----------------------------------------------------------------------
